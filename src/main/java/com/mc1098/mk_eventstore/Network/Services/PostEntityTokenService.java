 ...
 */
public class PostEntityTokenService implements EventStoreService
{
    
    private final EntityToken token;
    
    public PostEntityTokenService(EntityToken token)
    {
        this.token = token;
    }
    ...
    public ServiceResult execute(EventStore store)
    {
        ...
        {
            store.save(token);
            return new ServiceResult(this, "EntityToken saved.");
